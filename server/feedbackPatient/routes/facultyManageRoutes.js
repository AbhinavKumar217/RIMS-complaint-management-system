...
router.get("/department/:departmentId", async (req, res) => {
  try {
    const faculties = await Faculty.find({ department: req.params.departmentId }).populate("department");
    res.status(200).json(faculties);
  } catch (error) {
...
router.put("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.faculty && req.faculty._id.toString() !== req.params.id) {
      return res.status(403).json({ message: "Unauthorized to update this faculty" });
    }
...
router.delete("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.faculty && req.faculty._id.toString() !== req.params.id) {
      return res.status(403).json({ message: "Unauthorized to delete this faculty" });
    }
...
router.delete("/admin/:facultyId", authMiddleware, verifyAdmin, async (req, res) => {
    try {
      ...
      await sendEmail(faculty.email, "Profile Delete Notification", "Admin has deleted your account.");
